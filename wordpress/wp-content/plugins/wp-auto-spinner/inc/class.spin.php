...
class wp_auto_spin_spin {
	...
	function spin_wordai() {
		...
		$wp_auto_spinner_wordai_quality = trim ( get_option ( 'wp_auto_spinner_wordai_quality', 'Regular' ) );
		
		// migrate old quality from 1-100
		if (is_numeric ( $wp_auto_spinner_wordai_quality ))
			$wp_auto_spinner_wordai_quality = 'Regular';
		...
		if (trim ( $wp_auto_spinner_wordai_email ) != '' && trim ( $wp_auto_spinner_wordai_password ) != '') {
			...
			// curl post
			$curlurl = "http://wordai.com/users/turing-api.php";
			...
			// OPT_AUTO_SPIN_WORDAI_PERFECT
			if (in_array ( 'OPT_AUTO_SPIN_WORDAI_PERFECT', $opt )) {
				$curlpost = $curlpost .= '&perfect_tense=correct';
			}
			...
			if (stristr ( $exec, '{' )) {
				...
				if (isset ( $jsonReply->status )) {
					...
					if ($jsonReply->status == 'Success') {
						...
						// safe mode
						if (in_array ( 'OPT_AUTO_SPIN_WORDAI_SAFE', $opt )) {
	...
	function spin_sc() {
		...
		if (trim ( $wp_auto_spinner_sc_email ) != '' && trim ( $wp_auto_spinner_sc_password ) != '') {
			...
			if (stristr ( $exec, 'error=' )) {
				...
				// extract synonyms from remote post and replace in the original article
				$article = $this->find_remote_synonyms_and_replace ( $exec, $article );
	 ...
	 */
	function find_remote_synonyms_and_replace($remote_synonyms, $article) {
		...
		foreach ( $synonyms as $synonym_key => $synonym_val ) {
			...
			if (stristr ( $synonym_val, '|' )) {
				...
				$article = preg_replace ( '{\b' . preg_quote ( $synonym_val_parts [0] ) . '\b}u', '{' . $synonym_key . '}', $article, 1 );
